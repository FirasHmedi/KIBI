...
const playAnimalCardForBot = async (
  ...
  if (roundNb === 2) {
    ...
    emptySlots = botSlots
      .map(
        (
          slot: { cardId: string | undefined; canAttack: any },
          index: number
        ) => ({ slot, index })
      )
...
const canPlayKing = async (
  ...
  const roundNB = await getRoundNb(gameId);
  if (roundNB > 2) {
    const isKingOnBoard = botSlots.some((slot: { cardId: string }) =>
      isKing(slot.cardId)
    );
    if (isKingOnBoard) {
    ...
    if (!canplayking) {
      ...
      for (const kingCard of kingCards) {
        ...
        const filteredBotSlots = botSlots.filter(
          (slot: { cardId: string }) =>
            slot !== undefined && slot.cardId !== undefined
        );

        const slotIndexToSacrifice = filteredBotSlots.findIndex(
          (slot: { cardId: string; canAttack: any }) => {
...
const getDefendingAnimalIdAndSlot = async (
  ...
  for (const role of rolePriority) {
    const foundSlot = indexedSlots.find(
      (slot: { cardId: string | undefined; canAttack: any }) => {
...
const botAttack = async (gameId: string) => {
  ...
  if (roundNB > 2) {
    const slots = await getPlayerSlots(gameId);
    const ownerHasNoAnimals = slots.every(
      (slot: { cardId: string }) => slot.cardId === "empty"
    );
    ...
    if (player.canAttack && !ownerHasNoAnimals) {
      ...
      BotSlots.forEach(
        (slot: { cardId: string | undefined; canAttack: any }, index: any) => {
          const animalCard = getAnimalCard(slot.cardId);
      ...
      if (animalsThatCanAttack.length > 0) {
        ...
        if (target && animal) {
          const envLoadNb = await getItemsOnce(
            "/games/" + gameId + "/two/envLoadNb"
          );
...
const attemptAttackplayer = async (gameId: string) => {
  ...
  // If the attacker in element can attack the owner, perform the attack
    ...
    const attackerInElementIndex = botSlots.findIndex(
      (slot: { cardId: string | undefined; canAttack: any }) => {
        const animal = getAnimalCard(slot.cardId);
        return (
          animal && animal.role === "attacker" && animal.clan === elementType
        );
      }
    );
...
export const setElementForBot = async (gameId: string): Promise<boolean> => {
  ...
  for (const role of rolePriority) {
    const slotIndex = botSlots.findIndex(
      (slot: { cardId: string | undefined }) => {
        const animal = getAnimalCard(slot.cardId);
    ...
    if (slotIndex !== -1) {
      ...
      // Return true to indicate that the element was changed
    }
  }
...
export const executeBotTurn = async (gameId: string): Promise<void> => {
  ...
  if (bot.canAttack) {
    //this is an attempt so he can fail to attack so it is more suitable to not changing the element
